// import { RelationExpression } from '../../../src/queryBuilder/parsers/relationExpressionParser.ts';
import {} from '../../src/queryBuilder/parsers/relationExpressionParser.ts';
...
describe('RelationExpression', () => {
  ...
  function testClone(expr, cloned) {
    chai.expect(RelationExpression.create(expr).clone().node).to.containSubset(
      cloned,
    );
  }
  ...
  function testMerge(str1, str2, parsed) {
    it(str1 + ' + ' + str2 + ' --> ' + parsed, () => {
      expect(RelationExpression.create(str1).merge(str2).toString()).to.equal(
        parsed,
      );
      expect(
        RelationExpression.create(str1).merge(RelationExpression.create(str2))
          .toString(),
      ).to.equal(parsed);
    });
  }
  ...
  function testSubExpression(str, subStr) {
    it('"' + subStr + '" is a sub expression of "' + str + '"', () => {
      expect(RelationExpression.create(str).isSubExpression(subStr)).to.equal(
        true,
      );
    });
  }
